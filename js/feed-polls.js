...
(function() {
  ...
  const MIN_OPTIONS = 2, MAX_OPTIONS = 4;
  const SVG_PLUS = '<svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M12 5v14M5 12h14" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>';
  const SVG_X = '<svg width="14" height="14" viewBox="0 0 24 24" fill="none"><path d="M18 6L6 18M6 6l12 12" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>';
  ...
  window.createPoll = function(container) {
    ...
    const list = wrap.querySelector('.poll-builder-list');
    const addBtn = wrap.querySelector('.poll-builder-add');
    ...
    addBtn.addEventListener('click', function() {
      const count = list.querySelectorAll('.poll-builder-item').length;
    ...
    return {
      getData: function() {
        const items = list.querySelectorAll('.poll-builder-input');
        const options = [];
  ...
  window.showPollResults = async function(postId) {
    ...
    try {
      ...
      const votedIndex = localStorage.getItem('poll_voted_' + postId);
  ...
  function renderPollUI(postId, opts, votedIndex) {
    ...
    if (!card) return;
    const pollWrap = card.querySelector('.poll-opts');
    if (!pollWrap) return;
    ...
    opts.forEach(function(o, i) {
      ...
      const cls = 'poll-opt voted' + (i === votedIndex ? ' my-vote' : '');
      html +=
    ...
    const counter = pollWrap.nextElementSibling;
  ...
  document.addEventListener('click', function(e) {
    ...
    const optionIndex = parseInt(opt.getAttribute('data-option'), 10);
