...
async function loadAlliances() {
  try {
    const result = await window.sb.from('alliances')
      .select('*, leader:users!alliances_leader_id_fkey(id, name, avatar_url, dna_type, level)')
      .order('created_at', { ascending: false });

    if (result.error) throw result.error;
    allAlliances = result.data || [];

    const alIds = allAlliances.map(function(a) { return a.id; });
    if (alIds.length) {
      const mr = await window.sb.from('users').select('id, alliance_id').in('alliance_id', alIds);
      const mData = mr.data || [];
      allAlliances.forEach(function(a) {
        a.members = mData.filter(function(m) { return m.alliance_id === a.id; });
      });
    }
...
function buildAllianceCard(a, myId) {
  const leader = a.leader || {};
  const memberCount = a.members ? a.members.length : 0;
  const isMember = myId && a.members && a.members.some(function(m) { return m.id === myId; });
  ...
  let actionHtml = '';
  if (isLeader) { actionHtml = '<span class="al-card-role">Лидер</span>'; }
  else if (isMember) { actionHtml = '<button class="al-card-btn al-card-btn--leave" onclick="event.stopPropagation();allianceLeave(\'' + a.id + '\')">Выйти</button>'; }
  else { actionHtml = '<button class="al-card-btn al-card-btn--join" onclick="event.stopPropagation();allianceJoin(\'' + a.id + '\')">Вступить</button>'; }
  const logoHtml = a.logo_url ? '<img src="' + a.logo_url + '" alt="">'
  ...
  return '<div class="al-card glass-card"><div class="al-card-top">' +
    ...
    memberCount + ' участников</span>' + actionHtml + '</div></div>';
}
...
function renderAlliancesList(alliances) {
  ...
  if (!list) return;
  if (!alliances.length) { list.innerHTML = ''; if (empty) empty.classList.remove('hidden'); return; }
  if (empty) empty.classList.add('hidden');
  const myId = window.currentUser ? window.currentUser.id : null;
  list.innerHTML = alliances.map(function(a) { return buildAllianceCard(a, myId); }).join('');
}
