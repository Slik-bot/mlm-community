...
// SVG icons
var LB_SVG_CLOSE = '<svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M18 6L6 18M6 6l12 12" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>';
var LB_SVG_DOWN = '<svg width="20" height="20" viewBox="0 0 24 24" fill="none"><path d="M21 15v4a2 2 0 01-2 2H5a2 2 0 01-2-2v-4M7 10l5 5 5-5M12 15V3" stroke="currentColor" stroke-width="2" stroke-linecap="round"/></svg>';
var LB_SVG_LEFT = '<svg width="28" height="28" viewBox="0 0 24 24" fill="none"><path d="M15 18l-6-6 6-6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
var LB_SVG_RIGHT = '<svg width="28" height="28" viewBox="0 0 24 24" fill="none"><path d="M9 18l6-6-6-6" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
...
function _lbBindEvents(el) {
  try {
    ...
    var overlay = el.querySelector('.lb-overlay');
    var download = el.querySelector('.lb-download');
    var arrowL = el.querySelector('.lb-arrow-l');
    var arrowR = el.querySelector('.lb-arrow-r');
    var track = el.querySelector('.lb-track');
    var thumbs = el.querySelectorAll('.lb-th');
    ...
    var stageEl = el.querySelector('.lb-stage');
...
function _lbGoTo(index) {
  try {
    ...
    var track = _lb.querySelector('.lb-track');
    if (!track) return;
...
function _lbUpdateUI() {
  ...
  var cur = _lb.querySelector('.lb-cur');
  if (cur) cur.textContent = _lbIndex + 1;

  var thumbs = _lb.querySelectorAll('.lb-th');
  thumbs.forEach(function(th, i) {
    th.classList.toggle('lb-th-on', i === _lbIndex);
  });

  // Scroll active thumbnail into view
  var activeTh = _lb.querySelector('.lb-th-on');
  if (activeTh) {
