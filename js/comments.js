/* comments.js — комментарии MLM Community (premium) */
(function(){
'use strict';
...
window.openPostDetail = async function(postId) {
  ...
  try {
    var postResult = await sb.from('posts')
      .select('*, profiles(id, name, avatar_url, dna_type, level, is_verified)')
      .eq('id', postId).single();
...
function renderDetailScreen(scroll, post, comments) {
  ...
  roots.forEach(function(c) {
    ...
    if (reps && reps.length) {
      var rootName = (c.profiles || {}).name || 'Аноним';
  ...
  if (!roots.length) {
    html += '<div class="cmt-empty">Пока нет комментариев.<br>Будьте первым!</div>';
  }
...
window.likeComment = async function(id, el) {
  ...
  try {
    if (liked) {
      await sb.from('post_comment_likes').insert({ user_id: currentAuthUser.id, comment_id: id });
    } else {
      await sb.from('post_comment_likes').delete().match({ user_id: currentAuthUser.id, comment_id: id });
    }
  } catch(e) {}
};
...
function initCommentInput() {
  ...
  f.addEventListener('input', function() {
    ...
    btn.classList.toggle('active', this.value.trim().length > 0);
  });
  ...
  async function doSendComment() {
    ...
    cancelReply();
    try {
      var res = await sb.from('post_comments').insert({
      ...
      var nc = {id:res.data.id, user_id:currentAuthUser.id, content:text, created_at:res.data.created_at||new Date().toISOString(), parent_comment_id:parentId, profiles:prof};
