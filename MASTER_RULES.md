...
═══════════════════════════════════════════════════
13. ЗАЩИЩЁННЫЙ ФУНКЦИОНАЛ (НЕ ЛОМАТЬ!)
═══════════════════════════════════════════════════

Всё что работает на localhost — НЕ ТРОГАТЬ.

СПИСОК ЗАЩИЩЁННОГО:
1. Отправка сообщений
2. Цветные пузыри (ДНК-цвет входящих)
3. Галочки статусов (отправлено/доставлено/прочитано)
4. Realtime (новые сообщения появляются без перезагрузки)
5. Typing indicator
6. Статус «был X мин. назад» в шапке

ПРОТОКОЛ ПЕРЕД ЛЮБЫМ ИЗМЕНЕНИЕМ:
1. Проверить что функционал из списка выше не затронут
2. Если изменение может сломать что-то из списка — СТОП, сообщить пользователю
3. После изменения — проверить каждый пункт списка
4. Если что-то сломалось — немедленно git revert, не продолжать
